...
// 単語のデータの形を定義（日本語訳 exampleJp を追加）
interface Word {
  id: number;
  word: string;
  partOfSpeech: string;
      ...
      {words.length > 0 ? (
        <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-sm min-h-[450px] flex flex-col justify-between border border-gray-100">
          <div className="text-center">
            ...
            <p className="text-blue-500 font-bold text-sm uppercase tracking-wider">
              [{words[currentIndex].partOfSpeech}]
            </p>
          </div>

          {showDetail && (
            <div className="space-y-4 text-center bg-blue-50 p-6 rounded-2xl animate-in fade-in duration-300">
          ...
          <div className="flex gap-4 w-full mt-8">
            <button 
              ...
              className="flex-1 py-4 bg-gray-200 text-gray-900 rounded-xl font-bold hover:bg-gray-300 transition"
            >
              {showDetail ? "Hide" : "Answer"}
            </button>
            <button 
              onClick={nextCard} 
              className="flex-1 py-4 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition shadow-lg"
            >
              Next
            </button>
          </div>
        ...
        <div className="text-center bg-white p-10 rounded-3xl shadow-sm border">
          <p className="text-gray-400 mb-4">まだ単語が登録されていません</p>
          <Link href="/add" className="text-blue-500 font-bold underline">最初の単語を登録する</Link>
        </div>
      )}

      {words.length > 0 && (
        <Link href="/add" className="mt-10 text-gray-400 hover:text-blue-500 transition text-sm font-medium">
          ＋ 新しい単語を追加・一括登録
        </Link>
      )}
    </main>
